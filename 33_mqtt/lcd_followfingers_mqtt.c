...
void lcd_draw_line(int x_start, int y_start, int dir, int length, unsigned int color)
{
...
void lcd_fill_rectangle(int x_start, int y_start, int x_end, int y_end, unsigned int color)
{
    unsigned short rgb565_color = argb8888_to_rgb565(color);
    ...
    for(i = y_start; i <= y_end; i++, temp += width)
    {
        for(j = x_start; j <= x_end; j++)
            screen_base[temp + j] = rgb565_color;
    }
}
