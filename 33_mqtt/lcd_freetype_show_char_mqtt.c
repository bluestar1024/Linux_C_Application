...
int main(int argc, char *argv[])
{
    ...
    freetype_show_char(20, 50, L"资源列表:", 0xff00ff);

    char_fp = fopen("list.txt", "r");
    while(1)
    {
        if(fgets(str1, sizeof(str1), char_fp))
        {
            snprintf(str2, strlen(str1), "%s", str1);
            mbstowcs(china_str, str2, strlen(str2));
            freetype_show_char(20 + 328 * char_i, 100 + 50 * char_j, china_str, 0xff00ff);
            char_i++;
            if(char_i > 2)
            {
                char_i = 0;
