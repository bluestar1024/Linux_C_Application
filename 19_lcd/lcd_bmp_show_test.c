...
void show_bmp_image(char *path)
{
    ...
    int i = 0;
    int temp = 0;
    int temp_line_bytes = 0;
    int *temp_screen_base = NULL;
    int j = 0;
    int k = -1;
    ...
    temp_line_bytes = line_bytes / 3 * 4;
    printf("line_bytes: %d\n", line_bytes);
    printf("temp_line_bytes: %d\n", temp_line_bytes);
    ...
    if(line_length > temp_line_bytes)
        min_line_bytes = temp_line_bytes;
    else
    ...
    if(info_h.height > 0)
    {
        ...
        for(i = 0; i < min_height; i++, screen_base -= width)
        {
            ...
            temp_screen_base = screen_base;
            for(j = 0; j < min_line_bytes;)
            {
                if(j % 4 == 0)
                {
                    printf("j = %d, k = %d\n", j, k);
                    temp_screen_base[j] = 0x00;
                    j++;
                    k++;
                }
                else
                {
                    memcpy((char *)temp_screen_base + j, (char *)line_buf + 3 * k, 3);
                    j += 3;
                }
            }
        }
